...
const state = {
  ...
  lastQuery: "", // last submitted query — used by Retry button
  activeWs: null, // current WebSocket — held so Stop can close it
};
...
const autocompleteWrap = document.getElementById("autocompleteWrap");
...
document.addEventListener("click", (e) => {
  if (!autocompleteWrap.contains(e.target) && e.target !== queryInput) {
    acHide();
  }
});
...
function scheduleAutocomplete() {
  ...
  _acTimer = setTimeout(() => fetchAutocomplete(q), 700);
}
...
function acRender(items) {
  if (!items.length || state.isSearching) {
    acHide();
    return;
  }
  _acFocusIdx = -1;
  autocompleteList.innerHTML = items
    .map(
      (s, i) =>
        `<li class="autocomplete-item" data-idx="${i}" data-value="${s.replace(/"/g, "&quot;")}">` +
        `<span class="autocomplete-item-icon">🔍</span>` +
        `<span class="autocomplete-item-text">${s}</span></li>`,
    )
    .join("");
  autocompleteList.querySelectorAll(".autocomplete-item").forEach((el) => {
    el.addEventListener("mousedown", (e) => {
      e.preventDefault(); // prevent input blur
      acSelect(el.dataset.value);
    });
  });
  autocompleteWrap.classList.remove("hidden");
}

function acHide() {
  autocompleteWrap.classList.add("hidden");
  autocompleteList.innerHTML = "";
  _acFocusIdx = -1;
}

function acIsOpen() {
  return !autocompleteWrap.classList.contains("hidden");
}
...
function acMoveFocus(dir) {
  ...
  items[_acFocusIdx]?.classList.remove("active");
  _acFocusIdx = Math.max(-1, Math.min(items.length - 1, _acFocusIdx + dir));
  const active = items[_acFocusIdx];
  if (active) {
...
function acAcceptFocused() {
  const items = autocompleteList.querySelectorAll(".autocomplete-item");
  const active = items[_acFocusIdx] ?? items[0];
  if (active) acSelect(active.dataset.value);
}
...
function handleProgressEvent(event) {
  ...
  switch (event.type) {
    ...
    case "queries": {
      // Queries generated by lfm2.5-thinking:1.2b — show as numbered pills with model label
      const qs = event.queries || [];
      const planModel = event.model || "";
      if (rcProgressFill) rcProgressFill.style.width = "15%";

      // Close the previous step ("Planning search strategy…")
      if (rcActiveStep) {
        rcActiveStep.classList.remove("active");
        rcActiveStep.classList.add("done");
        const st = rcActiveStep.querySelector(".rc-step-status");
      ...
      // Build a step card showing each query as a numbered pill
      const step = document.createElement("div");
      step.className = "rc-step active";
      const queryPills = qs
        .map(
          (q, i) =>
            `<div class="rc-query-pill">` +
            `<span class="rc-query-num">${i + 1}</span>` +
            `<span>${q}</span>` +
            `</div>`,
        )
        .join("");
      step.innerHTML =
        ...
        `<div class="rc-step-text">` +
        `<div class="rc-step-label">${planModel || "Query planning"}</div>` +
        `<div class="rc-step-main">Generated ${qs.length} search quer${qs.length !== 1 ? "ies" : "y"}</div>` +
        `<div class="rc-query-grid">${queryPills}</div>` +
        `</div>` +
        `<div class="rc-step-status"><div class="step-mini-spin"></div></div>`;
...
function setBusy(busy) {
  ...
  if (busy) { clearTimeout(_acTimer); acHide(); }
  // Search ↔ Stop toggle
  btnSearch.classList.toggle("hidden", busy);
  btnStop.classList.toggle("hidden", !busy);
  // Retry only visible when idle and we have a previous query
  btnRetry.classList.toggle("hidden", busy || !state.lastQuery);
}
...
// ── Normal search (WebSocket with live progress and cancellation) ─────────────────
function runNormalSearch(query) {
  ...
  ws.onmessage = (ev) => {
    ...
    if (event.type === "answer" && event.content !== undefined) {
      ...
      state.activeWs = null;
      setBusy(false);
      if (rcSubtitle) rcSubtitle.textContent = "Search stopped";
...
// Stop button — sends cancel signal then closes the WebSocket
...
// Retry button — re-runs the last query
btnRetry.addEventListener("click", () => {
  if (state.lastQuery && !state.isSearching) {
    runNormalSearch(state.lastQuery);
  }
});
